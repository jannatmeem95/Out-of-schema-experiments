...
def readFile(model_name, dirname):
    metric = Metric()
    dir_path = 'output/'+ model_name + '/' + dirname + #'#alexaData/'#SGD'#MultiWoz'#alexaData'
    ...
    # return data
            ...
            with open(dir_path+'/scores/'+ name+ '_score.json', 'w') as out:
                json.dump(scores, out, indent=2)
            
            with open(dir_path+'/scores/tmpSet.json', 'w') as out:
