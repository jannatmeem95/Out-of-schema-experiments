...
def readFile():
    with open('/Users/jannatarameem/Desktop/Fall23/OQS/alexaData/out/testAlexa/testlogs.json', 'r') as f:
        val_data = json.load(f)

    with open('/Users/jannatarameem/Desktop/Fall23/OQS/alexaData/out/testAlexa/testlabels.json', 'r') as f:
        val_labels = json.load(f)
    
    print(len(val_data))
    preprocess(val_data, val_labels, 'gpt-4-test')

def preprocess(data, labels, name):
    out = open('logOut-4-alexa.txt', 'w')
    saved_results = dict()
    x = 0
    id = 1
    for k in range(len(data)):
        ...
        if id % 50 == 0 :
            print(id)
        id+=1
    with open('/Users/jannatarameem/Desktop/Fall23/ChatGPT/output/gpt-3.5/alexaData/GPT_output_'+ name + '.json', 'w') as f:
